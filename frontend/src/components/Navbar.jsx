            ...
            <div className="ml-8 flex items-center space-x-6">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`whitespace-nowrap text-sm font-bold transition-colors relative group py-1 ${
                    isActive(link.path) ? 'text-temple-gold' : 'text-white hover:text-temple-gold'
                  }`}
                >
                  {link.name}
                  <span className={`absolute bottom-0 left-0 h-0.5 bg-temple-gold transition-all ${
              ...
              <div className="flex items-center gap-3 pl-4 border-l border-white/20">
                  ...
                  <div className="flex items-center gap-3">
                    <Link to="/dashboard" className="flex items-center gap-2 group/user">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center border transition-all ${
                        isActive('/dashboard') ? 'bg-temple-gold border-temple-gold' : 'bg-temple-gold/20 border-temple-gold/30 group-hover/user:bg-temple-gold'
                      }`}>
                        <User size={14} className={isActive('/dashboard') ? 'text-temple-red' : 'text-temple-gold group-hover/user:text-temple-red'} />
                      </div>
                ...
                <Link 
                  to="/pooja"
                  className={`flex items-center gap-2 px-6 py-2 rounded-full text-sm font-bold transition-all shadow-md active:scale-95 whitespace-nowrap uppercase ${
                    isActive('/pooja') ? 'bg-white text-temple-red' : 'bg-temple-gold text-temple-red hover:bg-white'
                  }`}
                >
              ...
              <div className="grid grid-cols-1 gap-1">
                {navLinks.map((link) => (
                  <Link
                    key={link.path}
                    to={link.path}
                    className={`flex items-center justify-between px-4 py-4 rounded-xl text-base font-medium transition-all active:translate-x-2 ${
                      isActive(link.path) ? 'bg-white/10 text-temple-gold' : 'text-white/90 hover:bg-white/10 hover:text-temple-gold'
                    }`}
                    onClick={() => setIsOpen(false)}
                  >
                    {link.name}
                    <ArrowRight size={16} className={isActive(link.path) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 transition-all'} />
                  </Link>
                ))}
              </div>
